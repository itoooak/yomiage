...
impl Converter {
    pub(super) fn new(config: &Config, store: Store) -> Self {
        Self {
            client: Client::new(),
    ...
    pub(super) async fn convert(&self, target: &TargetConfig) -> anyhow::Result<ConversionStatus> {
        ...
        if let Err(error) = self.write_wav(input, &temporary_path).await {
    ...
    async fn write_wav(&self, input: &str, output_path: &Path) -> anyhow::Result<()> {
        let spec = WavSpec {
            channels: 1,
        ...
        for line in input.lines() {
            ...
            for sample in reader.into_samples::<i16>() {
                writer.write_sample(sample.context("failed to read synthesized WAV sample")?)?;
            }
